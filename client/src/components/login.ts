import "../stagewise.js";
import { html } from "../utils.js";

/**
 * 登录组件的模板定义
 * 包含样式和HTML结构
 */
const template = html`
    <style>
        /* 组件根元素样式 */
        :host {
            position: fixed;
            top: 0;
            left: 0;
            width: 100%;
            height: 100%;
            z-index: 1000;
            background-color: rgba(0, 0, 0, 0.5);
            display: flex;
            align-items: center;
            justify-content: center;
        }

        /* 遮罩层样式 */
        .overlay {
            position: absolute;
            top: 0;
            left: 0;
            width: 100%;
            height: 100%;
            cursor: pointer;
        }

        /* 登录容器样式 */
        .login-container {
            background: white;
            padding: 2rem;
            border-radius: 8px;
            box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1);
            width: 100%;
            max-width: 400px;
            position: relative;
        }

        /* 关闭按钮样式 */
        .close-btn {
            position: absolute;
            top: 1rem;
            right: 1rem;
            background: none;
            border: none;
            font-size: 1.5rem;
            color: #666;
            cursor: pointer;
            padding: 0.5rem;
            line-height: 1;
            transition: color 0.3s ease;
        }

        .close-btn:hover {
            color: #333;
        }

        /* 登录头部样式 */
        .login-header {
            text-align: center;
            margin-bottom: 2rem;
            padding-right: 2rem;
        }

        .login-header h1 {
            font-size: 1.8rem;
            color: #333;
            margin-bottom: 0.5rem;
        }

        .login-header p {
            color: #666;
            font-size: 0.9rem;
        }

        /* 表单组样式 */
        .form-group {
            margin-bottom: 1.5rem;
        }

        .form-group label {
            display: block;
            margin-bottom: 0.5rem;
            color: #333;
            font-size: 0.9rem;
        }

        .form-group input {
            width: 100%;
            padding: 0.75rem;
            border: 1px solid #ddd;
            border-radius: 4px;
            font-size: 1rem;
            transition: border-color 0.3s ease;
        }

        .form-group input:focus {
            outline: none;
            border-color: #4a90e2;
            box-shadow: 0 0 5px rgba(74, 144, 226, 0.2);
        }

        /* 登录按钮样式 */
        .login-btn {
            width: 100%;
            padding: 0.75rem;
            background-color: #4a90e2;
            color: white;
            border: none;
            border-radius: 4px;
            font-size: 1rem;
            cursor: pointer;
            transition: background-color 0.3s ease;
        }

        .login-btn:hover {
            background-color: #357abd;
        }

        /* 登录页脚样式 */
        .login-footer {
            text-align: center;
            margin-top: 1.5rem;
            font-size: 0.9rem;
            color: #666;
        }

        .login-footer a {
            color: #4a90e2;
            text-decoration: none;
        }

        .login-footer a:hover {
            text-decoration: underline;
        }

        /* 错误消息样式 */
        .error-message {
            color: #dc3545;
            font-size: 0.9rem;
            margin-top: 0.5rem;
            display: none;
        }

        .error-message.show {
            display: block;
        }
    </style>
    <div
        class="overlay"
        id="overlay"
    ></div>
    <div class="login-container">
        <button
            class="close-btn"
            id="closeBtn"
        >
            &times;
        </button>
        <div class="login-header">
            <h1>欢迎登录</h1>
            <p>登录以访问材料基因数据库的完整功能</p>
        </div>
        <form id="loginForm">
            <div class="form-group">
                <label for="username">用户名</label>
                <input
                    type="text"
                    id="username"
                    name="username"
                    required
                    placeholder="请输入用户名"
                />
            </div>
            <div class="form-group">
                <label for="password">密码</label>
                <input
                    type="password"
                    id="password"
                    name="password"
                    required
                    placeholder="请输入密码"
                />
            </div>
            <div
                class="error-message"
                id="errorMessage"
            ></div>
            <button
                type="submit"
                class="login-btn"
            >
                登录
            </button>
        </form>
        <div class="login-footer">还没有账号？<a href="/user/register.html">立即注册</a></div>
    </div>
`;

/**
 * 登录表单组件
 * 提供用户登录功能，包括：
 * - 用户名和密码输入
 * - 表单验证
 * - 登录请求处理
 * - 错误信息显示
 */
export class LoginForm extends HTMLElement {
    /** Shadow DOM 根节点 */
    readonly #sr = this.attachShadow({ mode: "open" });

    /**
     * 构造函数
     * 初始化组件，设置模板和事件监听
     */
    constructor() {
        super();
        this.#sr.innerHTML = template;
        this.init();
    }

    /**
     * 初始化组件
     * 设置表单提交、关闭按钮和遮罩层的点击事件
     */
    private init(): void {
        const form = this.#sr.getElementById("loginForm") as HTMLFormElement;
        const closeBtn = this.#sr.getElementById("closeBtn")!;
        const overlay = this.#sr.getElementById("overlay")!;
        const registerLink = this.#sr.querySelector(".login-footer a")!;

        form.addEventListener("submit", this.handleSubmit.bind(this));
        closeBtn.addEventListener("click", () => this.remove());
        overlay.addEventListener("click", () => this.remove());
        registerLink.addEventListener("click", (e) => {
            e.preventDefault();
            this.remove();
            const registerForm = document.createElement("register-form");
            document.body.appendChild(registerForm);
        });
    }

    /**
     * 处理表单提交
     * 发送登录请求并处理响应
     * @param event 表单提交事件
     */
    private async handleSubmit(event: Event): Promise<void> {
        event.preventDefault();

        const form = event.target as HTMLFormElement;
        const formData = new FormData(form);
        const username = formData.get("username") as string;
        const password = formData.get("password") as string;

        try {
            const response = await fetch("/api/user/login", {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
                },
                body: JSON.stringify({ username, password }),
            });

            const result = await response.json();

            if (response.ok) {
                // 登录成功，刷新页面
                window.location.reload();
            } else {
                // 显示错误信息
                const errorMessage = this.#sr.getElementById("errorMessage")!;
                errorMessage.textContent = result.message || "登录失败，请检查用户名和密码";
                errorMessage.classList.add("show");
            }
        } catch (error) {
            console.error("登录请求失败:", error);
            const errorMessage = this.#sr.getElementById("errorMessage")!;
            errorMessage.textContent = "网络错误，请稍后重试";
            errorMessage.classList.add("show");
        }
    }
}

// 注册自定义元素
customElements.define("material-login", LoginForm);
